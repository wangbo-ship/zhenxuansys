    ...
    <el-card>
      ...
      <!-- 分页器组件
        pagination
            ...
            layout:可以设置分页器六个子组件布局调整 ->表示将某些子组件顶到最右侧
            pager-count:设置中间的连续页码数
            current-change:页码发生变化时触发 默认会传递当前页码作为参数
            size-change:每页条数切换时触发 默认会传递当前选择的条数作为参数
      -->
      <el-pagination
...
//获取已有品牌的接口封装为一个函数:在任何情况下向其获取数据,调用该函数即可
...
const confirm = async () => {
  //在你发请求之前,要对于整个表单进行校验 ---validate是api自带的方法用于检测全部校验
  //调用这个方法进行全部表单相校验,如果校验全部通过 返回成功promise,才执行后面的语法
  ...
  if (result.code == 200) {
    ...
    //再次发请求获取已有全部的品牌数据 如果是修改就留在当前页 如果是添加就返回第一页
